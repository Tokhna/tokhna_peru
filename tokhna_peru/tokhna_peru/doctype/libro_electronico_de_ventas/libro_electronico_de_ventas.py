...
class LibroElectronicodeVentas(Utils):
    def get_sales_invoices(self, year, periodo):
        ...
        sales_invoices = frappe.db.sql("""select
				...
				IF(LENGTH(SUBSTRING(sales_invoice.name, LOCATE('-', sales_invoice.name) + 1, LOCATE('-',sales_invoice.name, LOCATE('-', sales_invoice.name) + 1) - LOCATE('-',sales_invoice.name) - 1))=3,CONCAT('0',SUBSTRING(sales_invoice.name, LOCATE('-', sales_invoice.name) + 1, LOCATE('-',sales_invoice.name, LOCATE('-', sales_invoice.name) + 1) - LOCATE('-',sales_invoice.name) - 1)), SUBSTRING(sales_invoice.name, LOCATE('-', sales_invoice.name) + 1, LOCATE('-',sales_invoice.name, LOCATE('-', sales_invoice.name) + 1) - LOCATE('-',sales_invoice.name) - 1)) as serie_comprobante,
				SUBSTRING(sales_invoice.name, LOCATE('-',sales_invoice.name, LOCATE('-', sales_invoice.name) + 1) + 1) as numero_comprobante,
				"" as resumen_diario,
